...
const Communities = () => {
  ...
  // Sample community data - this would be empty or from an API in a real app
  ...
  const handleSubmit = () => {
    ...
    const newCommunity = {
      ...
      likes: 0,
      members: [{ id: 1, name: "You", isAdmin: true }],
      flashcards: [],
      messages: [],
    }
  ...
  // Update the handleViewCommunity function to ensure we're getting the latest data

  const handleViewCommunity = (communityId) => {
    // Find the community from the current state, not the initial state
    const community = communities.find((c) => c.id === communityId)
